...
const InputVehicle: React.FC<Props> = ({ isChecked, setNumberOfVehicles, autocompleteValue, setAutocompleteValue }) => {
    ...
    useEffect(() => {
        setNumberOfVehicles(autocompleteValue.length);
    }, [autocompleteValue]);

    useEffect(() => {
        if (isChecked) {
            const vehiclesArrayFormated: any[] = [];
            vehiclesArray.map((vehicle: any) => {
                vehiclesArrayFormated.push(vehicle.vehicleImei);
            });
            setAutocompleteValue(vehiclesArrayFormated);
